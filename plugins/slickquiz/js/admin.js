        ...
        plugin.formHelper = {
            ...
            getValidQuizJson: function() {
                ...
                questions.each(function(i, questionSet) {
                    ...
                    selectAny         = $($(questionSet).find('.selectAny input[type="checkbox"]')).is(":checked");
                    forceCheckbox     = $($(questionSet).find('.forceCheckbox input[type="checkbox"]')).is(":checked");
                    question          = {"a": []};
                    correctAnswers    = false;

                    answers.each(function(i, answerSet) {
                        option  = $($(answerSet).children('input[type="text"]')[0]);
                        correct = ($($(answerSet).children('input[type="checkbox"]')).is(":checked"))? 'checked': '';
                    ...
                    if ($.inArray(false, questionValidations) > -1) {
                        ...
                        question["q"]              = questionInput.val();
                        question["correct"]        = correctResponse.val();
                        question["incorrect"]      = incorrectResponse.val();
