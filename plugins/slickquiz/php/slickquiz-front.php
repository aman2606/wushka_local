...
if ( !class_exists( 'SlickQuizFront' ) ) {
                        ...
                        $out .= '
                            <script>
                                jQuery(document).ready(function($) {
                                    (typeof quizNarrationState !== "undefined") ? quizNarration = quizNarrationState : quizNarration = true ;';
                        ...
                        if ( $this->get_admin_option( 'save_scores' ) == '1' ) {
                            ...
                            $out .='
                                    ...
                                    function submitScore' . $quiz->id . '() {
                                        var answers = $("ol.questions input[type=radio]:checked").map(function(i,e) {
                                            return {
                                                q:      $(e).attr("id").charAt($(e).attr("id").length-3),
                                                a:      $(e).attr("id").charAt($(e).attr("id").length-1),
                                                valid:  $(e).closest("li").attr("class")
                                            };
                                        }).get();
                                        var json = {
                                            name: nameLabel.val(),
	...
	{
	//	var_dump($id);exit;
            global $quiz, $status, $pageQuizzes, $current_user;

            $quizModel = new SlickQuizModel;
            $quiz = $quizModel->get_quiz_by_id( $id );
            $out  = '';
            $skip = '';
            if (user_can($current_user, "parent") || user_can($current_user, "teacher") || (user_can($current_user, "student") && (!isset($current_user->quizzes) || strtolower($current_user->quizzes) === 'optional'))) {
                $skip = '<div class="buttonWrapper"><button class="button" onclick="javascript:history.go(-2)">Skip Quiz</button></div>';
            }
