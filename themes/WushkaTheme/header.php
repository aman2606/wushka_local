<?php
// On YOOtheme builder pages (including the customizer preview iframe) use the
// parent theme's header so UIkit loads cleanly and the builder drag-and-drop works.
// WushkaTheme's Bootstrap JS conflicts with UIkit Sortable when both are present.
...
$wushka_template_url = get_stylesheet_directory_uri();
...
<head>
    ...
    <link rel="stylesheet" href="<?php echo $wushka_template_url; ?>/wushka-compiled.css?ver=6.01">
    ...
    <link rel="stylesheet" href="<?php echo get_stylesheet_directory_uri(); ?>/css/bookshelves.css?ver=1.01">
    ...
    <script>
        jQuery(document).ready(function($) {
            ...
            function ajax_set_class_session(i_class, s_archive) {
                $.ajax({
                    url: '<?php echo get_stylesheet_directory_uri() . '/set-class.php'; ?>',
                    type: "POST",
                    data: {
            ...
            ?>
                ...
                function loginAsStudent($id, $pw) {
                    $.ajax({
                        url: '<?php echo get_stylesheet_directory_uri() . '/login-as-student.php'; ?>',
                        type: "POST",
                        data: {
                            'id': $id,
                            'pw': $pw,
            ...
            ?>
                ...
                function selectDashboard(dashboard, redirect_url = '<?php echo get_home_url(); ?>') {
                    $.ajax({
                        url: '<?php echo get_stylesheet_directory_uri() . '/switch-dashboard.php'; ?>',
                        type: "POST",
                        data: {
                            'type': dashboard
                        },
