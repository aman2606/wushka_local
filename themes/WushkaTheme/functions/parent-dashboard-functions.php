...
//  $carousel_html  = get_carousel_html();
    ...
    // header
    $content_html .= '<div class="row mt15"> <div class="col-xs-12"> <h1 class="glyphicon-heading"> <span class="x2 glyphicon glyphicon-dashboard hidden-xs"></span> <span class="glyphicon-heading-text">Home Dashboard</span> </h1> </div> </div>';
    //Add Static Banner
    ...
    // Features
    $content_html .= "<section class='page-section teacher-functions padding-y grad-radial' id='section-functions'><div class='container'><div class='row'>$functions_html</div></div></section>";
    $content_html .= "</div>";
...
function teacher_functions_html() {
    ...
    $functions_html = '<div class="parent-functions">';
...
function build_teacher_functions_html($aFunctions = []) {
    ...
    foreach ($aFunctions as $aFunc) {
        //URL Link
        $item_html = '<div class="parent-functions-box col-xs-12 col-sm-4" id="' . $aFunc['id'] . '">';
        $item_html .= '<a href="' . $aFunc['url'] . '" class="parent-functions"><div class="parent-function wrapper grow">';
        //Heading
        $item_html .= '<div class="parent-function heading-wrapper"><span class="glyphicon parent-function icon"></span><h2 class="parent-function heading">' . $aFunc['title'] . '</h2></div>';
        //Dot Points
        $item_html .= '<div class="parent-function content"><p class="parent-function sub-heading">Here you can:</p><ul class="parent-function text-items">';
        foreach ($aFunc['points'] as $point) {
            $item_html .= '<li class="parent-function text-item"><span class="glyphicon glyphicon-play bullet-arrow"></span>' . $point . '</li>';
        }
        $item_html .= '</ul></div>';
        $item_html .= '</div></a></div>';
...
function get_carousel_html() {
    ...
    $a_carousel_html[] = '<a href="#" class="btn btn-default">My Latest Books</a>';
    $a_carousel_html[] = '<a href="#" class="btn btn-primary">Popular Books</a>';
    $a_carousel_html[] = '</div>';
    $a_carousel_html[] = '</div>';
    ...
    $a_carousel_html[] = '<div class="parent-dash-carousel-shelf"></div>';
    $a_carousel_html[] = '</div>';
    $a_carousel_html[] = '</div>';
    $a_carousel_html[] = '</div><!-- END row --->';
...
function generate_teacher_functions() {
    ...
    $teacher_functions[] = [
        ...
        'url' => home_url() . '/manage-child-list/',
        'points' => [
            'Add a child profile and link to their school',
            'Set your child\'s password',
    ...
    $teacher_functions[] = [
        ...
        'url' => home_url() . '/child-statistics/',
        'points' => [
            'View the reading progress of your child',
            'View the School Readers that have been completed'
        ]
    ];
//Library
    $teacher_functions[] = [
    ...
    $teacher_functions[] = [
        'id' => 'quiz-results',
        'title' => 'Comprehension Statistics',
        'url' => '/quiz-results',
        'points' => [
            'View the reading comprehension progress of your child',
            'View the results of completed Online Quizzes',
            'View the detailed responses for each quiz'
        ]
    ];
    ...
    $teacher_functions[] = [
        ...
        'url' => 'child-stories',
        'points' => [
            'View your child\'s stories',
            'Leave comments on their stories'
        ]
    ];*/
