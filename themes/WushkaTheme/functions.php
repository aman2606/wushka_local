...
// ---------------------------------------------------------------------------
// YOOtheme asset isolation
//
// On pages NOT built with the YOOtheme page builder, all YOOtheme/UIkit
// CSS and JS are suppressed so they never interfere with WushkaTheme's
// Bootstrap-based styles.
//
// Two-layer defence:
//   Layer 1 (priority 7) — removes assets via YOOtheme's Metadata API
//     before they are printed. Clean and zero-cost.
//   Layer 2 (priority 1 + 999) — output-buffers all of wp_head and
//     regex-strips any surviving <link>/<script> tags from the yootheme
...
// On builder/customizer pages, override theme.update.css which uses
// .uk-hidden-visually:not(:focus):not(:focus-within) — that pattern makes the
// YOOtheme accessibility skip link visible when the preview iframe has focus.
// We want it unconditionally hidden in the builder preview context.
add_action('wp_head', function () {
    if (!wushka_is_yootheme_builder_page()) return;
    echo '<style>.uk-hidden-visually{position:absolute!important;width:1px!important;height:1px!important;padding:0!important;border:0!important;margin:0!important;overflow:hidden!important;clip-path:inset(50%)!important;white-space:nowrap!important}</style>';
}, 99);

// Layer 1: suppress via YOOtheme's internal Metadata service (source-level).
...
add_action('wp_head', function () {
    ...
    try {
        ...
        foreach ([
            'style:theme', 'style:theme-update', 'style:theme-custom',
            'style:customizer-site', 'script:theme-uikit', 'script:theme-uikit-icons',
            'script:theme', 'script:theme-data', 'script:theme-search', 'script:theme-custom',
        ] as $key) {
            $meta->del($key);
        }
    } catch (\Throwable $e) {}
...
// Layer 2a: start output buffer at the very beginning of wp_head.
// $wushka_ob_head tracks whether WE started a buffer so Layer 2b never accidentally
...
// Layer 2b: close buffer and strip any remaining yootheme parent-theme tags.
add_action('wp_head', function () use (&$wushka_ob_head) {
    if (!$wushka_ob_head) return;
    $wushka_ob_head = false;
    $head = ob_get_clean();

    // <link> tags pointing to /themes/yootheme/ (negative lookahead skips yootheme-child)
    $head = preg_replace(
        '/<link\b[^>]*href=["\'][^"\']*\/themes\/yootheme\/(?!-)[^"\']*["\'][^>]*>\s*/i',
        '',
        $head
    );

    // <script src> tags pointing to /themes/yootheme/
    $head = preg_replace(
        '/<script\b[^>]*src=["\'][^"\']*\/themes\/yootheme\/(?!-)[^"\']*["\'][^>]*>.*?<\/script>\s*/is',
        '',
        $head
    );

    // Inline window.yootheme / yootheme.theme data block
    $head = preg_replace(
        '/<script[^>]*>\s*(?:window\.yootheme|yootheme\.theme)\b.*?<\/script>\s*/is',
        '',
        $head
    );

    // Inline <style> blocks with UIkit CSS custom properties (--uk-).
    $head = preg_replace(
        '/<style[^>]*>[^<]*--uk-[^<]*<\/style>\s*/is',
        '',
        $head
    ...
    // <style>@import 'url'</style> — YOOtheme outputs its theme CSS files using
    // @import inside a <style> block rather than a <link> tag in some builds.
    $head = preg_replace(
        '/<style[^>]*>\s*@import\s+["\'][^"\']*\/themes\/yootheme\/(?!-)[^"\']*["\'][^;]*;\s*<\/style>\s*/is',
        '',
        $head
    );
...
// Layer 2c: buffer wp_footer on non-builder pages.
// YOOtheme defers its UIkit JS (and sometimes CSS) to wp_footer, so the wp_head
...
// Layer 2d: close footer buffer and strip YOOtheme content.
add_action('wp_footer', function () use (&$wushka_ob_footer) {
    if (!$wushka_ob_footer) return;
    $wushka_ob_footer = false;
    $footer = ob_get_clean();
...
// Layer 3: safety-net — explicitly reassert the WushkaTheme rem base after all wp_head output.
// Fires outside the buffer (priority 1000 > buffer end 999) so it is never captured/filtered.
// Only applies on non-builder pages; builder pages use YOOtheme's 16px rem base.
add_action('wp_head', function () {
    if (!wushka_is_yootheme_builder_page()) {
        echo '<style>html{font-size:62.5%!important}</style>' . "\n";
    }
}, 1000);

// On YOOtheme builder pages, suppress all WushkaTheme-specific assets so only
// YOOtheme/UIkit CSS and JS load. The main compiled CSS is already excluded because
// header.php delegates to yootheme/header.php on builder pages (no <link> tag output).
// This hook removes the remaining enqueued assets (font-face declarations, jQuery UI,
// WushkaTheme utility scripts) and prevents the wp_footer lzk script from registering.
add_action('wp_enqueue_scripts', function () {
    if (!wushka_is_yootheme_builder_page()) return;

    // Font-face inline style block — not needed in YOOtheme context.
    wp_dequeue_style('wushka-fonts');
    wp_deregister_style('wushka-fonts');

    // jQuery UI components — conflict with UIkit Sortable/Draggable in the builder.
    foreach ([
        'jquery-validate', 'jquery-touchswipe',
        'jquery-ui-core', 'jquery-ui-widget', 'jquery-ui-mouse',
        'jquery-ui-accordion', 'jquery-ui-autocomplete', 'jquery-ui-slider',
        'jquery-ui-tabs', 'jquery-ui-sortable', 'jquery-ui-draggable',
        'jquery-ui-droppable', 'jquery-ui-resize', 'jquery-ui-dialog',
        'jquery-ui-button', 'jquery-touch-punch',
        'jquery-ajaxSubmit', 'ereader_script', 'wushka_buttons_script',
    ] as $handle) {
        wp_dequeue_script($handle);
    }

    // lessonzone_footer_scripts() enqueues jquery.lzk.js during wp_footer — remove it.
    remove_action('wp_footer', 'lessonzone_footer_scripts');
}, 100);

// The main stylesheet (wushka-compiled.css) is loaded via a single <link> tag in header.php,
// exactly as the original WushkaTheme did. Do NOT enqueue it here — doing so would load it
// twice (once in wp_head, once via the manual tag) and shift the CSS cascade order.
//
// This enqueue only registers a dummy handle so we can attach the @font-face overrides as
// an inline <style> block. The compiled CSS has hardcoded absolute font paths that break
// on subdirectory installs; this inline block overrides them with correct URLs.
add_action('wp_enqueue_scripts', function () {
    wp_register_style('wushka-fonts', false);
    wp_enqueue_style('wushka-fonts');

    $f = get_stylesheet_directory_uri() . '/fonts/';
    $font_css = "
@font-face { font-family:'DINWeb-Light'; src:url('{$f}DINWeb-Light.woff') format('woff'); }
@font-face { font-family:'DINWeb-Bold';  src:url('{$f}DINWeb-Bold.woff')  format('woff'); }
@font-face { font-family:'DINWeb';       src:url('{$f}DINWeb.woff')        format('woff'); }
@font-face {
    font-family:'LatoBold';
    src:url('{$f}lato-bold-webfont.eot');
    src:url('{$f}lato-bold-webfont.eot?#iefix') format('embedded-opentype'),
        url('{$f}lato-bold-webfont.woff2') format('woff2'),
        url('{$f}lato-bold-webfont.woff')  format('woff'),
        url('{$f}lato-bold-webfont.ttf')   format('truetype');
    font-weight:normal; font-style:normal;
}
@font-face {
    font-family:'LatoBoldItalic';
    src:url('{$f}lato-bolditalic-webfont.eot');
    src:url('{$f}lato-bolditalic-webfont.eot?#iefix') format('embedded-opentype'),
        url('{$f}lato-bolditalic-webfont.woff2') format('woff2'),
        url('{$f}lato-bolditalic-webfont.woff')  format('woff'),
        url('{$f}lato-bolditalic-webfont.ttf')   format('truetype');
    font-weight:normal; font-style:normal;
}
@font-face {
    font-family:'LatoItalic';
    src:url('{$f}lato-italic-webfont.eot');
    src:url('{$f}lato-italic-webfont.eot?#iefix') format('embedded-opentype'),
        url('{$f}lato-italic-webfont.woff2') format('woff2'),
        url('{$f}lato-italic-webfont.woff')  format('woff'),
        url('{$f}lato-italic-webfont.ttf')   format('truetype');
    font-weight:normal; font-style:normal;
}
@font-face {
    font-family:'LatoRegular';
    src:url('{$f}lato-regular-webfont.eot');
    src:url('{$f}lato-regular-webfont.eot?#iefix') format('embedded-opentype'),
        url('{$f}lato-regular-webfont.woff2') format('woff2'),
        url('{$f}lato-regular-webfont.woff')  format('woff'),
        url('{$f}lato-regular-webfont.ttf')   format('truetype');
    font-weight:normal; font-style:normal;
}
@font-face {
    font-family:'gooddogregular';
    src:url('{$f}gooddog-webfont-webfont.eot');
    src:url('{$f}gooddog-webfont-webfont.eot?#iefix') format('embedded-opentype'),
        url('{$f}gooddog-webfont-webfont.woff') format('woff'),
        url('{$f}gooddog-webfont-webfont.ttf')  format('truetype'),
        url('{$f}gooddog-webfont-webfont.svg#gooddogregular') format('svg');
    font-weight:normal; font-style:normal;
}";
    wp_add_inline_style('wushka-fonts', $font_css);
}, 5);
...
load_theme_textdomain('lessonzone', get_stylesheet_directory() . '/languages');
...
{
    wp_enqueue_script('lzk', get_stylesheet_directory_uri() . '/js/jquery.lzk.js', array('jquery'), NULL, TRUE);
...
function wushka_jquery_update_scripts()
{
    ...
    //wp_register_script('jquery', get_stylesheet_directory_uri() . '/js/jquery-3.5.1.js','','',false);  //Development version
    wp_register_script('jquery', get_stylesheet_directory_uri() . '/js/jquery-3.5.1.min.js', '', '', false);  //Production version

    //wp_register_script('jqueryMigrate', get_stylesheet_directory_uri() . '/js/jquery-migrate-3.3.1.js','','',false);  //Development version
    wp_register_script('jqueryMigrate', get_stylesheet_directory_uri() . '/js/jquery-migrate-3.3.1.min.js', '', '', false);  //Production version
